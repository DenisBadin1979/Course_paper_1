...
def profitable_cashback(file_path: str, number_mouth: str | int, number_year: str | int) -> str:
    """Функция для анализа выгодности категорий повышенного кешбэка.
    На вход функции поступают данные для анализа, год и месяц в формате строки."""
    try:
        mouth_year = ["01"]
        number_mouth_str = str(number_mouth)
        number_year_str = str(number_year)

        mouth_year.append(number_mouth_str)
        mouth_year.append(number_year_str)
