...
def test_reader_transaction_excel(mock_read_excel, mock_date):
    ...
    mock_date.datetime.strptime.return_value = datetime.datetime(2025, 4, 18)
    expected["Дата платежа"] = pd.to_datetime(expected["Дата платежа"], format="%d.%m.%Y")
    # print(expected.values)
    # print(reader_transaction_excel('e').values)
    assert reader_transaction_excel("a", "s").values == expected.values
...
def test_file_not_found(mock_logger):
    with pytest.raises(FileNotFoundError):
        reader_transaction_excel("non_existent_file.xlsx", "d")
...
def test_total_card_missing_column(mock_logger):
    ...
    df = pd.DataFrame({"Неправильная колонка": [1, 2, 3], "Другая колонка": ["a", "b", "c"]})
...
def test_total_transaction_success(mock_logger, sample_df):
    ...
    assert result == [
        {"date": "03.01.2023", "amount": 300.00, "category": "Развлечения", "description": "Кино"},
        {"date": "02.01.2023", "amount": 200.00, "category": "Транспорт", "description": "Такси"},
        {"date": "01.01.2023", "amount": 100.00, "category": "Еда", "description": "Покупка"},
    ]
...
class TestCurrencyConverter(unittest.TestCase):
    ...
    def test_successful_conversion(self, mock_getenv, mock_get):
        ...
        mock_response.json.return_value = {"rates": {"USD": 0.011, "EUR": 0.010}}
        ...
        self.assertEqual(
            result,
            [{"currency": "USD", "rate": round(1 / 0.011, 2)}, {"currency": "EUR", "rate": round(1 / 0.010, 2)}],
        )
...
class TestStockSP500(unittest.TestCase):
    ...
    def test_successful_stock_retrieval(self, mock_getenv, mock_get):
        ...
        mock_responses = [
            MagicMock(json=MagicMock(return_value={"Global Quote": {"01. symbol": "AAPL", "05. price": "175.43"}})),
            MagicMock(json=MagicMock(return_value={"Global Quote": {"01. symbol": "MSFT", "05. price": "338.11"}})),
        ]
        ...
        self.assertEqual(len(result), 2)
        self.assertEqual(result, [{"stock": "AAPL", "prices": "175.43"}, {"stock": "MSFT", "prices": "338.11"}])
